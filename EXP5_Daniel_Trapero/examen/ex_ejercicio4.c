...
int main (){
	...
	if (productos > 10){
		...
		for (int i = 0; i < productos; i;){
			printf("¿Desea quitar unidades algún producto? (si/no): ");
			scanf("%s",quitar); //Para quitar unidades, he creado una nueva función llamada quitar, en la que se resta de la cantidad total de unidades de la tienda 
			resultado = strcmp(quitar, comparacion);                                                                                                              
			if (resultado == 0){                          
				printf("Ingrese el nombre del producto: ");             
				scanf("%s",nombre);         
				printf("Cantidad a restar: ");           
				scanf("%d",&quitar);                
				tienda[i].cantidad = tienda[i].cantidad - quitar;            
			}else{                      
				if (resultado1 == 0){
					printf("Cantidad en stock: %d unidades\n",tienda[i].cantidad);                                                                                                                                           }else{
						printf("Este producto no existe\n");
						break;
						printf("%s - %d\n",tienda[i].nombre,tienda[i].cantidad);
